...
let ObjectStorageService = ObjectStorageService_1 = class ObjectStorageService {
    constructor() {
        this.logger = new common_1.Logger(ObjectStorageService_1.name);
        this.baseDir = path.resolve(__dirname, '..', '..', '..', 'uploads', 'community-attachments');
        fs.mkdirSync(this.baseDir, { recursive: true });
    }
    buildAttachmentKey(fileName) {
        const ext = (fileName.match(/\.([A-Za-z0-9]{1,8})$/)?.[1] || '').toLowerCase();
        return `community-attachments/${(0, crypto_1.randomUUID)()}${ext ? '.' + ext : ''}`;
    }
    async upload(key, buffer) {
        try {
            ...
            throw new common_1.InternalServerErrorException('فشل رفع الملف، حاول مرة أخرى');
        }
    }
    downloadStream(key) {
